...
// ── RBS type helpers ──────────────────────────────────────────────────────────
function rbsType(type, optional) {
    const base = rbsTypeBase(type);
    return optional ? `${base}?` : base;
}
function rbsTypeBase(type) {
        ...
        for (const m of svc.models) {
            ...
            for (const f of extractFields(m)) {
                const collectR = (t) => {
                    ...
                    if (isArrayType(t))
                        collectR(arrayElementType(t));
                    if (isRecordType(t))
                        collectR(recordElementType(t));
                };
                collectR(f.type);
            }
        }
